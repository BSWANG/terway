...
func (networkService *networkService) GetResourceMapping() (tracing.PodMappings, error) {
...
func toResMapping(poolStats tracing.ResourcePoolStats, pods []interface{}) (tracing.PodMappings, error) {
	...
	mapping := make(tracing.PodMappings, 0, len(all))
	for _, res := range all {
		// idle
