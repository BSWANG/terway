...
var _ = Describe("Node Controller", func() {
	...
	Context("EFLO node", func() {
		...
		It("new eflo node (eno)", func() {
			...
			Expect(resource.Labels["alibabacloud.com/lingjun-vpc-network-type-override"]).To(Equal("eno"))
		...
		It("should handle LinJunNetworkWorkKey label for EFLO node", func() {
			...
			efloController.On("DescribeNode", mock.Anything, mock.Anything).Return(&aliyunClient.DescribeNodeResponse{
				MachineType: "test-machine-type-normal",
			}, nil).Maybe()
		...
		It("should handle LinJunNetworkWorkKey label with high density ENI", func() {
			By("Creating k8s node")
			...
			// Mock DescribeNode response
			efloController.On("DescribeNode", mock.Anything, mock.Anything).Return(&aliyunClient.DescribeNodeResponse{
				MachineType: "test-machine-type-hd",
			}, nil).Maybe()
